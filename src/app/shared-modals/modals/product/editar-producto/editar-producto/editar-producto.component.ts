...
export class EditarProductoComponent implements OnInit {
  ...
  abrirModalIngredientes(): void {
    const dialogRef = this.dialog.open(MostrarIngredientesComponent, {
      width: '500px',
      data: {
        ingredientesActuales: structuredClone(this.ingredientesSeleccionados)
      }
    });
  ...
  abrirModalInsumos(): void {
    const dialogRef = this.dialog.open(MostrarInsumosComponent, {
      width: '500px',
      data: {
        insumosActuales: structuredClone(this.insumosSeleccionados)
      }
    });
  ...
  guardarProducto(): void {
    ...
    const imagenValida = typeof this.imagenProducto === 'string' &&
                         this.imagenProducto.startsWith('data:image');
    ...
    const productoActualizado = {
      ...
      ...(imagenValida && { imagen: this.imagenProducto })
    };
